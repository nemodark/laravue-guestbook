<template>
    <div class="card shadow mt-5">
        <div class="card-header py-3">
                <h3 class="m-0">
                    Update Guest <router-link to="/" class="btn btn-warning btn-sm float-right">Go Back</router-link>
                </h3>
            </div>
            <div class="card-body">
                <form @submit.prevent="updateGuest">
                    ...
                    <div class="form-group">
                        ...
                        <VuePhoneNumberInput v-model="guest.phone_number" @update="onPhoneUpdate" :default-country-code="defaultCountry" />
                    </div>
                    <div class="form-group">
    ...
    created() {
        this.axios.get(`http://guest-book.test/api/guests/edit/${this.$route.params.id}`)
            .then((response) => {
                ...
                this.defaultCountry = response.data.country_code;
        ...
        updateGuest() {
            this.axios.put(`http://guest-book.test/api/guests/update/${this.$route.params.id}`, this.guest)
                .then(response => {
                    ...
                    this.$toast.success({
                        title: 'Success',
                        message: `Guest ${this.guest.firstname} ${this.guest.lastname} updated successfully`
                    });
